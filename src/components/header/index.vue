    ...
    <div class=" fixed top-0 w-full h-16 bg-white z-10 header">
        <div class="relative w-full h-full flex items-center">
            ...
            <SvgIcon @click="router.forward()" iconClass="front" class="icon-Box absolute cursor-pointer rounded-lg"
                style="left: 15%;" />
            <div @mouseenter="enterIndex = 1" @mouseleave="enterIndex = 0" v-if="isLogin" class="absolute z-10 flex items-center duration-300 ease-out" style="right: 18%;" 
                :style="{
                    scale:enterIndex ?'2.5':'1',
                    transform:enterIndex ?'translate(-15%, 40%)':''
                }">
                <div 
                class=" rounded-full overflow-hidden w-8 cursor-pointer select-none" >
            ...
            <div v-if="!isLogin" @click="router.push({ path: '/login' })" class=" absolute cursor-pointer"  style="right: 18%;">
                <span>请登录</span>
            </div>
        </div>
    </div>
