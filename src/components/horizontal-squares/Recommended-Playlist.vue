        ...
        <div v-for="(item, index) in songList.values" :key="index" class="relative mb-4" style="width:18%">
            <div @click="toSongListPage(item.id)">
                <img @mouseenter="selectedIndex = index" @mouseleave="selectedIndex = 100"
                    class="mb-1 rounded-2xl overflow-hidden duration-500 ease-out select-none cursor-pointer"
            ...
            <div @click="toSongListPage(item.id)" @mouseenter="selectedIndex = index"
                class="w-12 h-12 rounded-full absolute duration-200 ease-out cursor-pointer"
            ...
            <div class=" flex items-center">
                <SvgIcon iconClass="play" class="mr-2"
                    style="width: 10px;right: 10%;color: var(--primary-back-color);" />
