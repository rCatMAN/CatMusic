<template>
    ...
    <div class="w-full flex items-start mt-8">
        <div v-for="(item, index) in similarAlbum.values" :key="index" style="width: 18%;margin-right: 5%;"
            class="relative">
            <el-image @mouseenter="selectedIndex = index" @mouseleave="selectedIndex = 100" @click="toAlbumPage(item.id)"
                :src="item.picUrl" class="w-full cursor-pointer z-10" lazy />
...
onMounted(async () => {
    ...
    for (let item of artistAlbumRes.hotAlbums) {
        console.log('similarAlbum', similarAlbum)
    ...
    console.log('artistAlbumRes', artistAlbumRes)
})
