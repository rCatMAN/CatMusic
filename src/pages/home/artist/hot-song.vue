<template>
  ...
  <div
    class="w-full flex items-center justify-start flex-wrap mt-4 overflow-hidden"
    :style="{ height: isShow === true ? 'auto' : '162px' }"
  >
    <div
      @click="playSong(item.id)"
      v-for="(item, index) in artistHotSong.values"
      :key="index"
    ...
    >
      ...
      <div class="ml-3 flex flex-col items-start justify-center w-full truncate">
        <div class="w-full flex items-center truncate">
          ...
          <div
            v-if="item.fee === 1"
            class="vip-icon w-7 h-4 ml-2 rounded-sm flex items-center justify-center"
            :style="{
              backgroundColor:
  ...
  <div class="mt-4">
    <span
      @click="isShow = !isShow"
      class="title text-sm font-bold text-gray-500 duration-200 ease-out cursor-pointer"
